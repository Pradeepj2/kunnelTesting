...
const StickyHeadTable = ({ compensationPendingList }) => {
  const rows = compensationPendingList;
  ...
  const approve = (data) => {
    let Data = {
      compensation_list: [data],
      status: "A",
    };

    axios
      .post(
        `${process.env.REACT_APP_API_URL}/wagemanage/compensation/approval_change/`,
        Data,
        {
          headers: {
...
const mapStateToProps = (state) => ({
  compensationPendingList: state.compensationPendingList,
});
